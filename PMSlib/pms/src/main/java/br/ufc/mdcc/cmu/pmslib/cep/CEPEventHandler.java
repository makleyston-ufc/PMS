    ...
    public void registerEPL() {
        for (Class stmSb : resourceClasses) {
            try {
                ...
                ConfigREST o = new ConfigREST();

                Class[] parameterType = new Class[1];
                parameterType[0] = Object.class;
                Object obj =  Class.forName(stmSb.getName())
                        .getDeclaredConstructor(parameterType).newInstance(o);


//                CEPResource cepResource = ((CEPResource)stmSb.newInstance());
                EPStatement epStatement = epService.getEPAdministrator().createEPL(((CEPResource)obj).getStatement());
                this.cepResources.add((CEPResource) obj);
                epStatement.setSubscriber(obj);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
